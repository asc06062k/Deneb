...
const dimOf = (n) => layer(S8, n).encoding.opacity.condition.value;
check("dim levels: points 0.5, labels 0.3, connector 0.2; default opacity 1 everywhere", dimOf("point_actual_hit_target") === 0.5 && dimOf("point_reference") === 0.5 && dimOf("label_actual") === 0.3 && dimOf("label_reference") === 0.3 && dimOf("connector_rule") === 0.2 && S8.layer.filter((l) => l.encoding.opacity).every((l) => l.encoding.opacity.value === 1));
check("chapter text states the three levels (points 0.5, labels 0.3, connector 0.2)", md.includes("จุด (0.5) ป้ายตัวเลข (0.3) และ Connector (0.2)") && md.includes("**0.3**") && md.includes("**0.2**"));
