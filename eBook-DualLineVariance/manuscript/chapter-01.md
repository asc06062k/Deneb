...
| พ.ย. 2021 | รุ่น **1.0.0**: [changelog รุ่นเก่า](https://deneb.guide/docs/archive/changelog-002) ระบุวันที่ 13 พ.ย. 2021 ส่วน GitHub release เผยแพร่วันที่ 24 พ.ย. 2021 |
...
| ความยืดหยุ่นด้านรูปแบบ | จำกัดตามตัวเลือกใน Format pane | สูงที่สุด แต่ยังอยู่ภายใต้ Power BI Custom Visual API, sandbox และข้อกำหนดการรับรอง | สูง ในขอบเขตที่ Vega/Vega-Lite บรรยายได้ และภายใต้ข้อจำกัดของรุ่นที่ผ่านการรับรอง (เช่น โหลดรูปจาก URL ภายนอกไม่ได้) |
...
![Dual-Line Variance Chart ระหว่างพัฒนา (spec rev 5) แสดงยอดขายจริงเทียบเป้าหมาย 12 เดือน บน Power BI Desktop](../images/chapter-01/CH01-S01-final-visual-powerbi.png)

*ภาพที่ 1-1 ภาพหน้าจอจริงจาก Power BI Desktop 2.157.1354.0 และ Deneb 2.0.0.0 ถ่ายระหว่างทดสอบเมื่อ 24 ก.ย. 2026 ขณะใช้ **spec rev 5** (ก่อนฉบับสุดท้าย rev 7): Visual ตั้งขนาด 800×450 แสดงยอดขายจริง (Actual) เทียบเป้าหมาย (Reference) รายเดือนของชุดข้อมูล Workshop ไฟล์ภาพถูก crop เหลือ 556×318 px — rev 7 เปลี่ยนเฉพาะการจัดวางชื่อบนแกน X (อ่านชื่อจาก Category, ชื่อแรกไม่ชิดขอบซ้าย) และรูปแบบการจางเมื่อมี Cross-highlight ส่วนอื่นเหมือนเดิม*
...
- **Cross-filtering (ขาออก)**: คลิกจุดของเดือนใดเดือนหนึ่งบน Dual-Line Variance Chart เพื่อส่ง selection ไปยัง Visual อื่น ซึ่งจะแสดงผลเป็น filter หรือ highlight ตามที่ตั้งไว้ใน `Edit interactions` ของ Power BI
- **Cross-highlighting (ขาเข้า)**: เมื่อเลือกเดือนใน Visual อื่นที่ตั้ง `Edit interactions` เป็น Highlight จุด เส้นเชื่อม และป้ายตัวเลขของเดือนอื่นจะจางลง
...
3. **ข้อมูลต้องเตรียมด้วย Power Query และคำนวณจุดตัดไว้ตอน Refresh เท่านั้น** พื้นที่ที่แบ่งสีตรงจุดตัดต้องใช้แถวข้อมูลเพิ่มที่คำนวณไว้ล่วงหน้า (บทที่ 4) ผลที่ตามมามี 3 ข้อ
   - **Slicer หรือ Filter ที่ตัดบางเดือนออกจาก Visual นี้** จะไม่ทำให้คำนวณจุดตัดใหม่ พื้นที่สีของช่วงก่อนหน้าจะ "ค้าง" ยื่นไปถึงตำแหน่งของเดือนที่ถูกกรองออก ขณะที่เส้นลากข้ามไปเดือนถัดไป (ทดสอบบน Power BI จริงแล้ว) Visual รุ่นนี้จึงออกแบบมาสำหรับหน้ารายงานที่ไม่มี Slicer ตัดเดือนแบบ interactive ถ้าจำเป็นต้องมี บทที่ 9 จะแนะนำทางเลือกสำรองแบบ "ทาสีทั้งช่วง" ที่ทำงานถูกต้องกับ Filter ทุกแบบ แต่เปลี่ยนสีที่กึ่งกลางช่วงแทนจุดตัดจริง
   - **ใช้ได้กับแกน Y แบบเส้นตรง (linear) เท่านั้น** จุดตัดคำนวณใน value space เชิงเส้น ถ้าเปลี่ยนแกน Y เป็น log scale หรือ scale แบบไม่เป็นเส้นตรง ตำแหน่งที่แบ่งสีจะไม่ตรงกับจุดที่เส้นตัดกันจริง
   - **Template ไม่ plug-and-play** ใช้กับข้อมูลใหม่ได้ก็ต่อเมื่อเตรียมตารางด้วย Power Query แบบเดียวกัน (บทที่ 9)
...
- Dual-Line Variance Chart ของเล่มนี้มี 7 ส่วนประกอบหลัก ทำงานร่วมกับ Visual อื่นได้ และมีข้อจำกัดที่รู้ล่วงหน้า 4 ข้อ (ข้อ 3 แยกเป็น 3 ผลกระทบ)
