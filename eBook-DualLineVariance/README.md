...
| 5 กราฟสองเส้นแรก | **PASS** (25 ก.ย. 2026, Codex รอบแรก ไม่มี Mandatory) ภาพจริง 5-1 ถึง 5-7 (ภาพ 5-3 ถ่ายใหม่แล้ว), JSON excerpt ตรวจกับ specs/steps ด้วย `qa/scripts/run-ch05-claims-check.mjs` (26 ผ่าน) คงบรรทัด `$schema` ตามคำตัดสินผู้ใช้ | `review/chapters/CH05_CODEX_VERDICT.md` (PASS) |
| 6 พื้นที่ Variance และ Connector | **PASS** (26 ก.ย. 2026, Codex รอบแรก ไม่มี Mandatory) ภาพจริง 6-1, 6-2, 6-4, 6-5, 6-6 ภาพ 6-3 เป็นภาพตัด-ขยายจาก 6-1/6-2 (ระบุใน caption) JSON excerpt ตรวจกับ specs/steps ด้วย `qa/scripts/run-ch06-claims-check.mjs` (31 ผ่าน) Step 4 สลับ Lower is Good ที่ Power Query ทดสอบจริง เส้นขอบประ Bad ใน Lower is Good ยังไม่มีภาพ | `review/chapters/CH06_CODEX_VERDICT.md` (PASS) |
| 7 Data Label และ Tooltip | **PASS** (26 ก.ย. 2026, Codex รอบ 2: รอบ 1 REVISE 1 ข้อ CH07-M01 คือหมายเหตุ Category ชื่อยาวส่งบทที่ 9) ภาพจริง 7-1 ถึง 7-7 ภาพ 7-8 เป็นภาพตัด-ขยายจาก 7-7 (ระบุใน caption) JSON excerpt และตัวเลขตรวจด้วย `qa/scripts/run-ch07-claims-check.mjs` (34 ผ่าน) ที่ Visual 280×180 ป้ายเหลือเดือน 1, 4, 7, 10, 12 (`labelStep` 3) ผู้เขียนประมาณผิดเป็น 2 ก่อนถ่ายภาพ บทเล่าตามจริง ไม่มีภาพ Reference point ไม่มี Tooltip (ผู้ใช้รายงาน) แบบฝึกหัดยังไม่ได้ลอง | `review/chapters/CH07_CODEX_VERDICT.md` (REVISE) → `..._R2.md` (PASS) |
| 8 Cross-filtering และ Cross-highlighting | **PASS** (26 ก.ย. 2026, Codex รอบ 2: รอบ 1 REVISE เรื่องภาพ 3 ข้อ M1–M3) ภาพจริง 8-1 ถึง 8-17 JSON excerpt และพฤติกรรม opacity ตรวจด้วย `qa/scripts/run-ch08-claims-check.mjs` (24 ผ่าน รันนิพจน์จริงกับแถวจำลอง) พบและแก้บั๊กต้นทาง: บทที่ 4 Step 4 ต้อง Rename แถบ `Sum of Actual`/`Sum of Reference` เป็น `Actual`/`Reference` (แก้บทที่ 4 แล้ว ภาพ 4-7 ถ่ายใหม่ 4-8 เป็นภาพตัดขยาย) | `review/chapters/CH08_CODEX_VERDICT.md` (REVISE) → `..._R2.md` (PASS) |
| 9–10 | ยังไม่เริ่ม | – |

**Phase 4 backlog (ต้องเสร็จก่อนเผยแพร่)**: (1) เปลี่ยน opacity พื้นที่ Good/Bad เป็น **0.15 ทั้งเล่ม** (คำตัดสินผู้ใช้ 26 ก.ย. 2026 แก้ final spec, static-test spec, template, Step ทั้ง 11 ไฟล์แล้ว) จึงต้องถ่ายภาพที่เห็นพื้นที่สีใหม่ทั้งหมด: ภาพ 1-1 และภาพในบทที่ 6, 7, 8 ที่ยังถ่ายด้วย 0.35 หรือ 0.25 (ยกเว้น 8-3, 8-6, 4-7 ที่ถ่ายด้วยค่าใหม่แล้ว) พร้อมลบกล่องหมายเหตุ "ภาพถ่ายก่อนเปลี่ยน opacity" ในบท 6, 7, 8 (2) ถ่ายภาพ 1-1 ใหม่จาก spec สุดท้าย (ค้างจากบทที่ 1) (2b) ภาพ 8-12 และ 8-13 ถ่ายก่อนแยกระดับความจาง (26 ก.ย. 2026 ผู้ใช้กำหนด จุด 0.5 ป้ายตัวเลข 0.3 Connector 0.2 แก้ spec/Step/template/ชุดทดสอบแล้ว Codex รอบ 3 PASS) ถ่ายรวมในรอบเดียวกับข้อ (1) (3) ลองแบบฝึกหัดที่ระบุว่า "ยังไม่ได้ลอง" ในบทที่ 5–8 บน Deneb จริงหรือคงข้อความ
